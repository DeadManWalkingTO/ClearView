...
class CDP {
    ...
    ConnectToYouTubeTabWithRetry(maxWaitMs := 6000) {
        ...
        try {
            ...
            ; Polling στο /json μέχρι να εμφανιστεί το tab
    ...
    EvaluateWithRetry(expr, attempts := 5, delayMs := 500) {
        ...
        while (i <= attempts) {
            ...
            if (raw != "") {
                ; προσπαθούμε να εξάγουμε numeric "value"
    ...
    _sendRecv(obj, timeoutMs := 1500) {
        ...
        local json, ok, expectedId, tStart, raw, packet, idGot, r
        ...
        while (A_TickCount - tStart < timeoutMs) {
            ...
            if (raw = "")
                break
            ; Προσπαθούμε να βρούμε "id": <num> στο raw
            idGot := this._jsonExtractNumber(raw, "id")
            if (idGot != "" && (idGot + 0) = expectedId) {
                r["__raw"] := raw
                return r
            }
            ; αν ήταν notification ή άλλο μήνυμα, συνεχίζουμε
        }
        return r
    }
...
class WSClient {
    static Connect(wsUrl) {
        ...
        try {
            ; Parse URL και ορισμός secure flag
            ...
            ; Δημιουργία αιτήματος GET με WebSocket upgrade
            ...
            ; Προσθήκη βασικών WebSocket headers
            ...
            ; Αποστολή request & λήψη response
            ...
            ; Ολοκλήρωση WebSocket upgrade → λαμβάνουμε WS handle
            ...
            ; Το hRequest πλέον δεν χρειάζεται
            DllCall("winhttp\WinHttpCloseHandle", "ptr", hRequest)

            ; Επιστροφή composite αντικειμένου με τα handles
            ws := Map()
            ws["session"] := hSession
            ws["connect"] := hConnect
            ws["ws"] := hWS
            ...
            ; Προσπάθεια καθαρισμού σε περίπτωση μερικών handles
            try {
    ...
    static Close(ws) {
        ; Θωράκιση: βεβαιώσου ότι είναι Map/Object με τα αναμενόμενα πεδία
        if (!IsObject(ws)) {
            return
        }

        ; 1) Προσπάθησε ευγενικό WebSocket close (status 1000)
        try {
            if (ws.Has("ws") && ws["ws"]) {
            ...
            ; προαιρετικό: log
            ; this.log?.Write(Format("WS graceful close err: {} @{}:{}", e.Message, e.File, e.Line))
        }

        ; 2) Κλείσε τα handles με ασφάλεια (ws -> connect -> session)
        try {
            if (ws.Has("ws") && ws["ws"]) {
